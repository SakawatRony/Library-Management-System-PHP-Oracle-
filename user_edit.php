...
<?php
	...
	<title>Edit</title>
     ...
     <body>
      <form method="post" action="user_edit_controller.php" class="login-box">
                 <?php 
                session_start();

                if(isset($_SESSION['user_edit_error'])=='error')
                {  echo '<p style="margin-left:80px;">'.'Please Fill out all the Blanks!!'.'</p>';
                    //header('location: index.php');
                    unset($_SESSION['user_edit_error']);
                 }
                ?>
	  <input type="hidden" name="username" value ="<?php echo ociresult($query, "USERNAME")?>" required=""/>
	<p>Name</p>
	<input type="text" name="name" value="<?php echo ociresult($query, "NAME")?>" required="" >
	<p>Email</p>
	<input type="text" name="email" value="<?php echo ociresult($query, "EMAIL")?>" required="">
	<br/> <br/>
	<input type="submit" name="submit" value="Update">
    <center><a href="user_list.php" class="already">Back</a></center>
     </form>
    </body>
   </html>
<?php		
